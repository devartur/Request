    ...
    <c:if test="${not empty requestScope.allrequest}">
		...
		<c:choose>
		    <c:when test="${requests.request==0}">
		        <button class="well well-sm centered" data-toggle="modal" data-target="#myModal${requests.id}">
  ZŁOŻONY
</button>
		    </c:when>
 ...
 ODRZUCONY
		    ...
		    <c:otherwise>
		      <div class="well well-sm centered"><c:out value="INNY" /></div>
		      	...
		      	<!-- Modal do wyskakującego okna -->
<div class="modal fade" id="myModal${requests.id}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Zamknij</span></button>
        <h4 class="modal-title" id="myModalLabel">Dokonaj wyboru</h4>
      </div>
      <div class="modal-body">
        <p>Wniosek numer: <c:out value="${requests.id}" /></p>
      </div>
      <div class="modal-footer">
        <form action="acceptRequest" method="post">
        <input type="hidden" name="requestid" value="${requests.id}">
<input type="submit" class="btn btn-default" name="option" value="Odrzucam">
<input type="submit" class="btn btn-primary" name="option" value="Potwierdzam">
		        ...
		        <div class="col col-md-10 col-sm-9">
		          ...
		          Data końca: <c:out value="${requests.stop}" />, ID wniosku: <c:out value="${requests.id}"/> </small></h6>
